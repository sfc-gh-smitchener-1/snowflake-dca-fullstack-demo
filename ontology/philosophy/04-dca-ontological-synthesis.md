...
- `sql/11-15` — Infrastructure, tables, population, inference, sharing (root demo)
- `ontology/demo/` — Snowflake-native ontology reference architecture (triple
  store, property graph, Graph RAG, Cortex Analyst)
- Full documentation: [docs/ONTOLOGY.md](../../docs/ONTOLOGY.md)

> **Note:** the earlier Neo4j sidecar / SPCS REST API and the dual-backend
> Knowledge Graph visualization have been removed for now (can be re-added later).
