...
Whiteboard: Sources → Knowledge Graph (Nodes + Edges in Snowflake) → **Snowflake-native query engine** (recursive CTEs + label propagation) → Outputs (Scores, Rings, Traversals, Reports).

> "The graph of record lives in two Snowflake tables, and the engine that reads them is pure SQL. Recursive CTEs and label propagation handle fraud-ring detection, AML scoring, corridor and agent scoring, and sanctions traversal — including beneficial-ownership chains to a sanctioned ultimate beneficial owner and shortest-path queries on the payment authorization path. No sidecar to operate, no extra license, everything inside your Snowflake security perimeter."
...
```sql
-- Real-time path query at payment authorization time — recursive CTE in the warehouse
CALL DCA_DEMO.GOVERNANCE.SP_GRAPH_SHORTEST_PATH('{customer_id}', '{watchlist_id}', 5);
```

> "Real-time. No batch. The graph finds indirect connections that name-matching never would. A recursive CTE traces the shortest path from the customer to the watchlist entity in tens of milliseconds without ever leaving the warehouse — and it handles the deeper beneficial-ownership chains too, all in pure SQL with nothing else to operate."
...
| "Why not just use a dedicated graph database?" | "For fraud-ring detection, AML scoring, corridor/agent scoring, and sanctions traversal — including multi-hop beneficial-ownership chains — Snowflake-native recursive CTEs are plenty fast and carry zero extra operational cost. You avoid standing up, securing, and syncing a separate graph database. The graph of record stays in Snowflake, and every query runs inside your existing security perimeter." |
