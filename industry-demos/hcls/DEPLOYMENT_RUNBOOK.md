...
- [ ] **Staffing-outcomes correlations computed:**
  ```sql
  SELECT COUNT(*) FROM SEM_DEV.HCLS_ANALYTICS.HCLS_CORRELATION_RESULTS;
  -- Expected: > 0
  ```

- [ ] **Comorbidity analysis populated:**
  ```sql
  SELECT COUNT(*) FROM SEM_DEV.HCLS_ANALYTICS.HCLS_PATIENT_COMORBIDITY;
  -- Expected: > 0
  ```

- [ ] **Payer metrics computed:**
  ```sql
  SELECT COUNT(*) FROM SEM_DEV.HCLS_ANALYTICS.HCLS_PAYER_METRICS;
  -- Expected: > 0
  ```

- [ ] **Comorbidity pairs identified:**
  ```sql
  SELECT COUNT(*) FROM SEM_DEV.HCLS_ANALYTICS.HCLS_COMORBIDITY_PAIRS;
  -- Expected: > 0
  ```

- [ ] **Care gaps detected:**
  ```sql
  SELECT COUNT(*) FROM SEM_DEV.HCLS_ANALYTICS.HCLS_CARE_GAPS;
  -- Expected: > 0
  ```
