...
-- Siemens Desigo CC/MindSphere — acquired company's 2K data centers
CREATE SCHEMA IF NOT EXISTS RAW_DEV.SIEMENS_DCIM
    COMMENT = 'Siemens Desigo CC/MindSphere — acquired portfolio (2K DCs, BMS, power, cooling, fire)';

-- DATA_ENGINEER: full control on Siemens schema
GRANT ALL ON SCHEMA RAW_DEV.SIEMENS_DCIM TO ROLE DATA_ENGINEER;

-- DATA_STEWARD: read-only
GRANT USAGE ON SCHEMA RAW_DEV.SIEMENS_DCIM TO ROLE DATA_STEWARD;
GRANT SELECT ON ALL TABLES IN SCHEMA RAW_DEV.SIEMENS_DCIM TO ROLE DATA_STEWARD;
GRANT SELECT ON FUTURE TABLES IN SCHEMA RAW_DEV.SIEMENS_DCIM TO ROLE DATA_STEWARD;
