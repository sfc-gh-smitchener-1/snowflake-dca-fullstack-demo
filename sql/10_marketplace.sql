...
(
    ...
    'SELECT PO_TYPE, SUM(TOTAL_SPEND) as TOTAL_SPEND, SUM(PO_COUNT) as PO_COUNT FROM SEM_DEV.MARKETPLACE.DP_SAP_PROCUREMENT_ANALYTICS GROUP BY 1 ORDER BY 2 DESC',
    'Spend analysis by PO type',
    'SELECT PO_YEAR, PO_MONTH, ROUND(AVG(AVG_PO_VALUE), 2) as AVG_VALUE FROM SEM_DEV.MARKETPLACE.DP_SAP_PROCUREMENT_ANALYTICS GROUP BY 1, 2 ORDER BY 1, 2',
...
(
    ...
    'SELECT STATUS, SUM(TOTAL_DEBITS) as DEBITS, SUM(TOTAL_CREDITS) as CREDITS FROM SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS WHERE FISCAL_YEAR = YEAR(CURRENT_DATE()) GROUP BY 1',
    'YTD activity by status for trial balance',
    'SELECT PERIOD_NAME, SUM(JOURNAL_ENTRY_COUNT) as ENTRIES, SUM(BATCH_COUNT) as BATCHES FROM SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS GROUP BY 1 ORDER BY 1',
    'Period close activity tracking',
    'SELECT LEDGER_ID, SUM(NET_AMOUNT) as NET FROM SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS GROUP BY 1',
    'Net amount by ledger',
    'SELECT FISCAL_YEAR, SUM(TOTAL_DEBITS) as DEBITS FROM SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS GROUP BY 1 ORDER BY 1 DESC',
    'Annual debit totals',
...
(
    ...
    'SELECT INVOICE_TYPE, SUM(INVOICE_COUNT) as COUNT, SUM(TOTAL_INVOICE_AMOUNT) as AMOUNT FROM SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS GROUP BY 1',
    'Invoice type distribution analysis',
    'SELECT APPROVAL_STATUS, SUM(UNPAID_COUNT) as UNPAID, SUM(TOTAL_INVOICE_AMOUNT) as AMOUNT FROM SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS GROUP BY 1 ORDER BY 2 DESC',
    'Unpaid invoices by approval status',
    'SELECT SOURCE, SUM(INVOICE_COUNT) as COUNT FROM SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS GROUP BY 1',
    'Invoice count by source',
    'SELECT INVOICE_YEAR, SUM(CANCELLED_COUNT) as CANCELLED FROM SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS GROUP BY 1 ORDER BY 1 DESC',
...
(
    ...
    'CRM_CONSUMER', 'CRM_CONSUMER, MARKETPLACE_CONSUMER, DATA_ADMIN',
    'SELECT ACCOUNT_TIER, SUM(ACCOUNT_COUNT) as ACCOUNTS, SUM(TOTAL_ARR) as ARR FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH GROUP BY 1 ORDER BY 2 DESC',
    'Account and ARR distribution by tier',
    'SELECT INDUSTRY, SUM(ACTIVE_ACCOUNTS) as ACTIVE, AVG(RETENTION_RATE_PCT) as RETENTION FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH GROUP BY 1 ORDER BY 3',
    ...
    'SELECT INDUSTRY, SUM(TOTAL_EMPLOYEES) as EMPLOYEES FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH GROUP BY 1 ORDER BY 2 DESC',
    'Employee distribution by industry',
    'SELECT ACCOUNT_TIER, AVG(AVG_ARR) as AVG_ARR FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH GROUP BY 1',
...
(
    ...
    'SELECT CASE_TYPE, SUM(CASE_COUNT) as CASES FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_SERVICE_ANALYTICS GROUP BY 1 ORDER BY 2 DESC',
    'Case volume by type',
    'SELECT CASE_STATUS, SUM(CLOSED_COUNT) as CLOSED FROM SEM_DEV.MARKETPLACE.DP_SALESFORCE_SERVICE_ANALYTICS GROUP BY 1',
...
(
    ...
    'SELECT REASON, SUM(ENCOUNTER_COUNT) as VISITS FROM SEM_DEV.MARKETPLACE.DP_FHIR_CLINICAL_ENCOUNTERS WHERE ENCOUNTER_YEAR = YEAR(CURRENT_DATE()) GROUP BY 1 ORDER BY 2 DESC',
    ...
    'SELECT ENCOUNTER_STATUS, SUM(ENCOUNTER_COUNT) as COUNT FROM SEM_DEV.MARKETPLACE.DP_FHIR_CLINICAL_ENCOUNTERS GROUP BY 1',
...
(
    ...
    'SELECT COUNTRY, SUM(PATIENT_COUNT) as PATIENTS FROM SEM_DEV.MARKETPLACE.DP_FHIR_POPULATION_HEALTH GROUP BY 1',
    'Patient count by country',
    'SELECT AGE_BAND, GENDER, SUM(PATIENT_COUNT) as PATIENTS FROM SEM_DEV.MARKETPLACE.DP_FHIR_POPULATION_HEALTH GROUP BY 1, 2 ORDER BY 1, 2',
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE SAP_ERP_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE SAP_ERP_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_SALES_ANALYTICS TO SHARE SAP_ERP_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_PROCUREMENT_ANALYTICS TO SHARE SAP_ERP_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_CUSTOMER_SUMMARY TO SHARE SAP_ERP_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE ORACLE_FINANCIALS_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE ORACLE_FINANCIALS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS TO SHARE ORACLE_FINANCIALS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS TO SHARE ORACLE_FINANCIALS_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE SALESFORCE_CRM_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE SALESFORCE_CRM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_PIPELINE TO SHARE SALESFORCE_CRM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH TO SHARE SALESFORCE_CRM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_SERVICE_ANALYTICS TO SHARE SALESFORCE_CRM_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE FHIR_HEALTHCARE_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE FHIR_HEALTHCARE_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_CLINICAL_ENCOUNTERS TO SHARE FHIR_HEALTHCARE_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_POPULATION_HEALTH TO SHARE FHIR_HEALTHCARE_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_CONDITION_ANALYTICS TO SHARE FHIR_HEALTHCARE_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE WORKDAY_HR_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE WORKDAY_HR_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_WORKFORCE TO SHARE WORKDAY_HR_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_COMPENSATION TO SHARE WORKDAY_HR_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_TIME_OFF TO SHARE WORKDAY_HR_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE SERVICENOW_ITSM_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE SERVICENOW_ITSM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_INCIDENTS TO SHARE SERVICENOW_ITSM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_CHANGES TO SHARE SERVICENOW_ITSM_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_PROBLEMS TO SHARE SERVICENOW_ITSM_DATA_SHARE;
...
GRANT USAGE ON DATABASE SEM_DEV TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT USAGE ON SCHEMA SEM_DEV.MARKETPLACE TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- SAP
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_SALES_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_PROCUREMENT_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SAP_CUSTOMER_SUMMARY TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- Oracle
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_ORACLE_FINANCIAL_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_ORACLE_AP_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- Salesforce
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_PIPELINE TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_ACCOUNT_HEALTH TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SALESFORCE_SERVICE_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- FHIR
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_CLINICAL_ENCOUNTERS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_POPULATION_HEALTH TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_FHIR_CONDITION_ANALYTICS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- Workday
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_WORKFORCE TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_COMPENSATION TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_WORKDAY_TIME_OFF TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- ServiceNow
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_INCIDENTS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_CHANGES TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.DP_SERVICENOW_PROBLEMS TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
-- Catalog
GRANT SELECT ON VIEW SEM_DEV.MARKETPLACE.VW_DATA_PRODUCT_CATALOG TO SHARE ENTERPRISE_ANALYTICS_DATA_SHARE;
...
-- Note: The following commands create listings in your private data exchange.
-- Replace 'YOUR_DATA_EXCHANGE_NAME' with your actual internal data exchange name.
-- You can find your data exchange name in Snowsight under Data > Private Sharing > Data Exchanges.

-- To create listings, run these commands after setting up your data exchange:
-- 
-- CREATE LISTING SAP_ERP_ANALYTICS_LISTING
--     FOR SHARE SAP_ERP_DATA_SHARE
--     AS $$
--     title: "SAP ERP Analytics"
--     subtitle: "Sales, Procurement, and Customer Analytics from SAP"
--     description: |
--         Comprehensive SAP ERP analytics providing real-time insights into:
--         
...
--     terms_of_service: "Internal use only. Data must not be shared externally."
--     business_needs:
--         - "Revenue Reporting"
...
--     $$
--     PUBLISH = FALSE  -- Internal only, not public
--     DATA_EXCHANGE_NAME = 'YOUR_DATA_EXCHANGE_NAME';
