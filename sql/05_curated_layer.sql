...
--   CURATED_DEV.WORKDAY_HCM - Workday HCM healthcare-specific dimensions and facts
--   CURATED_DEV.PAYER       - Payer/claims adjudication dimensions and facts
--
-- RUN AS: DATA_ADMIN
-- ============================================================================
...
CREATE SCHEMA IF NOT EXISTS CURATED_DEV.WORKDAY_HCM
    COMMENT = 'Workday HCM healthcare-specific curated dimensions and facts';

CREATE SCHEMA IF NOT EXISTS CURATED_DEV.PAYER
    COMMENT = 'Payer/claims adjudication curated dimensions and facts';
...
-- ═══════════════════════════════════════════════════════════════════════════
-- WORKDAY HCM (HCLS Healthcare-Specific) CONFIGURATION
-- ═══════════════════════════════════════════════════════════════════════════

INSERT INTO CURATED_DEV.SHARED.CURATED_CONFIG 
    (SOURCE_SYSTEM, TARGET_TABLE, TABLE_TYPE, SOURCE_TABLE, SELECT_SQL, TARGET_LAG, TABLE_COMMENT)
VALUES
('WORKDAY_HCM', 'DIM_WORKERS', 'DIMENSION', 'WORKERS', '
SELECT
    WORKER_ID,
    EMPLOYEE_ID,
    FIRST_NAME,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.WORKERS
', '1 hour', 'Workday HCM healthcare workers - nurses, physicians, techs'),

('WORKDAY_HCM', 'DIM_DEPARTMENTS', 'DIMENSION', 'DEPARTMENTS', '
SELECT
    DEPARTMENT_ID,
    DEPARTMENT_NAME,
    UNIT_TYPE,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.DEPARTMENTS
', '1 hour', 'Clinical departments and nursing units'),

('WORKDAY_HCM', 'FACT_SHIFTS', 'FACT', 'SHIFTS', '
SELECT
    SHIFT_ID,
    WORKER_ID,
    DEPARTMENT_ID,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.SHIFTS
', '1 hour', 'Actual worked shifts with staffing metrics'),

('WORKDAY_HCM', 'FACT_STAFFING_ASSIGNMENTS', 'FACT', 'STAFFING_ASSIGNMENTS', '
SELECT
    ASSIGNMENT_ID,
    WORKER_ID,
    DEPARTMENT_ID,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.STAFFING_ASSIGNMENTS
', '1 hour', 'Worker-to-unit staffing assignments'),

('WORKDAY_HCM', 'DIM_CERTIFICATIONS', 'DIMENSION', 'CERTIFICATIONS', '
SELECT
    CERT_ID,
    WORKER_ID,
    CERTIFICATION_TYPE,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.CERTIFICATIONS
', '24 hours', 'Clinical certifications (BLS, ACLS, CCRN, etc.)'),

('WORKDAY_HCM', 'FACT_TIME_OFF', 'FACT', 'TIME_OFF', '
SELECT
    REQUEST_ID,
    WORKER_ID,
    DEPARTMENT_ID,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.TIME_OFF
', '1 hour', 'Time off requests with coverage impact'),

('WORKDAY_HCM', 'FACT_TURNOVER', 'FACT', 'TURNOVER_EVENTS', '
SELECT
    EVENT_ID,
    WORKER_ID,
    DEPARTMENT_ID,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.TURNOVER_EVENTS
', '24 hours', 'Staff turnover events with reason codes'),

('WORKDAY_HCM', 'FACT_COMPENSATION', 'FACT', 'COMPENSATION_HISTORY', '
SELECT
    COMP_ID,
    WORKER_ID,
    EFFECTIVE_DATE,
    ...
    "_SOURCE_SYSTEM",
    "_ROW_HASH"
FROM RAW_DEV.WORKDAY_HCM.COMPENSATION_HISTORY
', '24 hours', 'Compensation history with change tracking');

-- ═══════════════════════════════════════════════════════════════════════════
-- PAYER (Claims Adjudication) CONFIGURATION  
-- ═══════════════════════════════════════════════════════════════════════════

INSERT INTO CURATED_DEV.SHARED.CURATED_CONFIG 
    (SOURCE_SYSTEM, TARGET_TABLE, TABLE_TYPE, SOURCE_TABLE, SELECT_SQL, TARGET_LAG, TABLE_COMMENT)
VALUES
('PAYER', 'DIM_PLANS', 'DIMENSION', 'PLANS', '
SELECT
    PLAN_ID, PLAN_NAME, PLAN_TYPE, PAYER_NAME, NETWORK_TIER,
    ANNUAL_DEDUCTIBLE, MAX_OOP, COPAY_PRIMARY, COPAY_SPECIALIST,
    COPAY_EMERGENCY, COINSURANCE_RATE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.PLANS
', '24 hours', 'Insurance plan definitions'),

('PAYER', 'DIM_MEMBERS', 'DIMENSION', 'MEMBERS', '
SELECT
    MEMBER_ID, PATIENT_ID, PLAN_ID, SUBSCRIBER_ID, GROUP_NUMBER,
    EFFECTIVE_DATE, TERMINATION_DATE, RELATIONSHIP, PCP_PRACTITIONER_ID,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.MEMBERS
', '1 hour', 'Plan membership linked to FHIR patients'),

('PAYER', 'FACT_COVERAGE', 'FACT', 'COVERAGE_PERIODS', '
SELECT
    COVERAGE_ID, MEMBER_ID, PLAN_ID, START_DATE, END_DATE,
    COVERAGE_TYPE, STATUS,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.COVERAGE_PERIODS
', '1 hour', 'Enrollment/coverage history'),

('PAYER', 'FACT_CLAIMS_DETAIL', 'FACT', 'CLAIMS_DETAIL', '
SELECT
    CLAIM_ID, CLAIM_LINE_ID, MEMBER_ID, PATIENT_ID, ENCOUNTER_ID,
    SERVICE_DATE, CPT_CODE, CPT_DESCRIPTION, ICD10_PRIMARY, ICD10_SECONDARY,
    DRG_CODE, PLACE_OF_SERVICE, RENDERING_PROVIDER_NPI, BILLING_PROVIDER_NPI,
    BILLED_AMOUNT, ALLOWED_AMOUNT, PAID_AMOUNT, COPAY, COINSURANCE,
    DEDUCTIBLE_APPLIED, PATIENT_RESPONSIBILITY, CLAIM_STATUS,
    DENIAL_REASON_CODE, DENIAL_REASON_DESCRIPTION, ADJUDICATION_DATE,
    DAYS_TO_ADJUDICATE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.CLAIMS_DETAIL
', '1 hour', 'Line-level claims with adjudication'),

('PAYER', 'FACT_PRIOR_AUTH', 'FACT', 'PRIOR_AUTHORIZATIONS', '
SELECT
    AUTH_ID, MEMBER_ID, REQUESTING_PROVIDER_NPI, PROCEDURE_CODE,
    PROCEDURE_DESCRIPTION, DIAGNOSIS_CODE, DIAGNOSIS_DESCRIPTION,
    URGENCY, STATUS, REQUEST_DATE, DECISION_DATE, DAYS_TO_DECISION,
    APPROVED_UNITS, APPROVED_DURATION_DAYS, DENIAL_REASON,
    APPEAL_STATUS, REVIEWER_ID, CLINICAL_RATIONALE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.PRIOR_AUTHORIZATIONS
', '1 hour', 'Prior authorization requests and decisions'),

('PAYER', 'FACT_UTIL_REVIEWS', 'FACT', 'UTILIZATION_REVIEWS', '
SELECT
    REVIEW_ID, MEMBER_ID, ENCOUNTER_ID, REVIEW_TYPE,
    LEVEL_OF_CARE_REQUESTED, LEVEL_OF_CARE_APPROVED, REVIEW_DATE,
    REVIEWER_TYPE, CLINICAL_CRITERIA, DETERMINATION,
    APPROVED_DAYS, ACTUAL_DAYS, VARIANCE_DAYS, COST_SAVINGS_ESTIMATE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.UTILIZATION_REVIEWS
', '1 hour', 'Medical necessity utilization reviews'),

('PAYER', 'FACT_PLAN_OF_CARE', 'FACT', 'PLAN_OF_CARE', '
SELECT
    POC_ID, MEMBER_ID, ENCOUNTER_ID, CONDITION_CODE, CONDITION_DESCRIPTION,
    GOAL_DESCRIPTION, INTERVENTION_TYPE, RESPONSIBLE_PROVIDER_NPI,
    START_DATE, TARGET_END_DATE, ACTUAL_END_DATE, PAYER_APPROVED,
    APPROVED_VISITS, USED_VISITS, STATUS, OUTCOME_MEASURE, OUTCOME_VALUE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.PLAN_OF_CARE
', '1 hour', 'Care plans with payer approval tracking'),

('PAYER', 'FACT_QUALITY_MEASURES', 'FACT', 'QUALITY_MEASURES', '
SELECT
    MEASURE_ID, MEMBER_ID, MEASURE_CODE, MEASURE_NAME,
    MEASUREMENT_YEAR, NUMERATOR_FLAG, DENOMINATOR_FLAG, EXCLUSION_FLAG,
    VALUE, BENCHMARK, STAR_RATING, GAP_IN_CARE,
    "_SOURCE_SYSTEM", "_ROW_HASH"
FROM RAW_DEV.PAYER.QUALITY_MEASURES
', '24 hours', 'HEDIS/STAR quality measures per member');
...
$$
    var results = [];
    var sourceSystems = ['SAP', 'SALESFORCE', 'ORACLE', 'FHIR', 'WORKDAY', 'SERVICENOW', 'WORKDAY_HCM', 'PAYER'];
...
GRANT USAGE ON SCHEMA CURATED_DEV.WORKDAY_HCM TO ROLE DATA_ENGINEER;
GRANT USAGE ON SCHEMA CURATED_DEV.PAYER TO ROLE DATA_ENGINEER;
...
GRANT SELECT ON ALL DYNAMIC TABLES IN SCHEMA CURATED_DEV.WORKDAY_HCM TO ROLE DATA_ENGINEER;
GRANT SELECT ON ALL DYNAMIC TABLES IN SCHEMA CURATED_DEV.PAYER TO ROLE DATA_ENGINEER;
...
GRANT SELECT ON FUTURE DYNAMIC TABLES IN SCHEMA CURATED_DEV.WORKDAY_HCM TO ROLE DATA_ENGINEER;
GRANT SELECT ON FUTURE DYNAMIC TABLES IN SCHEMA CURATED_DEV.PAYER TO ROLE DATA_ENGINEER;
...
--   CALL CURATED_DEV.SHARED.BUILD_CURATED_LAYER('WORKDAY_HCM');
--   CALL CURATED_DEV.SHARED.BUILD_CURATED_LAYER('PAYER');
--
